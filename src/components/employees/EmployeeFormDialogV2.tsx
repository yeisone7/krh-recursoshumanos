...
import { useCreateEmployeeV2, useUpdateEmployeeV2 } from '@/hooks/useEmployeesV2';
...
export function EmployeeFormDialogV2({ open, onOpenChange, employee, onSuccess }: EmployeeFormDialogV2Props) {
  const [activeTab, setActiveTab] = useState('identity');
  const { currentCompanyId } = useAuth();
  const { data: operationCenters = [] } = useOperationCenters();
  const { data: areas = [] } = useAreas();
  const { data: positions = [] } = usePositions();
  const createEmployee = useCreateEmployeeV2();
  const updateEmployee = useUpdateEmployeeV2();
