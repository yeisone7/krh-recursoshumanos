...
export function EducationLevelFormDialog({ open, onOpenChange, educationLevel }: EducationLevelFormDialogProps) {
  ...
  // Reset form when dialog opens or educationLevel changes
  useEffect(() => {
    if (open) {
      if (educationLevel) {
        form.reset({
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[90dvh] w-[calc(100vw-2rem)] flex-col overflow-hidden sm:max-w-md">
        <DialogHeader className="shrink-0">
          <DialogTitle>{isEditing ? 'Editar Nivel Educativo' : 'Nuevo Nivel Educativo'}</DialogTitle>
        </DialogHeader>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Ej: Profesional, Especialización..." {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
                    ...
                    <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                      <div className="space-y-0.5">
                        ...
                        <div className="text-sm text-muted-foreground">
                          Indica si este nivel educativo está disponible para selección.
                        </div>
                      </div>
                      <FormControl>
                        <Switch
