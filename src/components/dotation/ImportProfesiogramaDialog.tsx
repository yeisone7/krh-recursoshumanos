          ...
          {!isParsed ? (
            <div className="space-y-6">
              <label className="group relative flex flex-col items-center justify-center py-16 border-2 border-dashed border-border/50 rounded-[2rem] cursor-pointer hover:border-primary/40 hover:bg-primary/[0.02] transition-all duration-300">
                <div className="p-5 rounded-2xl bg-background group-hover:bg-primary/10 transition-colors">
              ...
              <div className="rounded-2xl border border-border/50 bg-background p-5 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {['Centro de Operación', 'Cargo', 'Código Artículo', 'Cantidad'].map((col) => (
                    <div key={col} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-background border border-border/30">
                  ...
                  <TableBody>
                    {parsedData.map((prof, idx) => (
                      <TableRow key={idx} className={cn(
                        ...
                        prof.status === 'skip_invalid' && 'opacity-50 grayscale bg-background /5'
                      )}>
                        <TableCell className="px-4 py-3">
                          ...
                          {prof.status === 'update' && (
                            <Badge className={cn(
                              ...
                              upsertMode ? "bg-primary/10 text-primary" : "bg-background text-muted-foreground"
                            )}>
                        ...
                        <TableCell className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">
                            {prof.items.slice(0, 2).map((item, i) => (
                              <Badge key={i} variant="secondary" className="h-5 px-1.5 rounded bg-background font-bold text-[8px] uppercase tracking-tight text-foreground">
