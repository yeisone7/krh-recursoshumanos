...
export default function CopasstAnalytics() {
  ...
  const exportCenterParticipation = async () => {
    if (!analytics.data || !selectedElection) return;
    try {
      const XLSX = await import('xlsx');
      const rows = buildCenterParticipationExportRows(analytics.data.segments.center);
      const sheet = XLSX.utils.json_to_sheet(rows);
      sheet['!cols'] = [{ wch: 36 }, { wch: 18 }, { wch: 14 }, { wch: 12 }, { wch: 12 }];
      rows.forEach((_, index) => {
        const cell = sheet[`B${index + 2}`];
        if (cell) cell.z = '0.0%';
      });
      const book = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(book, sheet, 'Participación por centro');
      const term = selectedElection.term_label.replace(/[^a-zA-Z0-9áéíóúÁÉÍÓÚñÑ_-]+/g, '-');
      XLSX.writeFile(book, `participacion-centros-copasst-${term}.xlsx`);
      await logCopasstExport(electionId, 'center_participation_xlsx');
      toast.success('Participación por centro exportada');
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'No fue posible exportar la participación por centro');
    }
  };
  ...
  return <div className="space-y-6 p-4 sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-3xl font-bold">Analítica COPASST</h1><p className="text-muted-foreground">Resultados agregados y participación por dimensiones congeladas.</p></div>{canExport(COPASST_PERMISSIONS.analytics) && <Button variant="outline" onClick={exportPdf} disabled={!analytics.data}><Download className="mr-2 h-4 w-4" />Generar acta PDF</Button>}</div>
    ...
    {analytics.data && <><CopasstKpis summary={analytics.data.kpis} />
      ...
      <Tabs defaultValue="gender"><TabsList className="flex h-auto flex-wrap"><TabsTrigger value="gender">Sexo</TabsTrigger><TabsTrigger value="center">Centro</TabsTrigger><TabsTrigger value="area">Área</TabsTrigger><TabsTrigger value="position">Cargo</TabsTrigger></TabsList><TabsContent value="gender" className="grid gap-4 lg:grid-cols-2"><SegmentChart data={analytics.data.segments.gender} /><GenderVotePie data={analytics.data.segments.gender} /></TabsContent><TabsContent value="center"><SegmentTable title="Participación por centro de operación" segmentLabel="Centro de operación" data={analytics.data.segments.center} onExport={canExport(COPASST_PERMISSIONS.analytics) ? exportCenterParticipation : undefined} /></TabsContent><TabsContent value="area"><SegmentChart data={analytics.data.segments.area} /></TabsContent><TabsContent value="position"><SegmentTable title="Participación por cargo" segmentLabel="Cargo" data={analytics.data.segments.position} /></TabsContent></Tabs>
...
function GenderVotePie({ data }: { data: CopasstSegment[] }) {
  const distribution = buildGenderVoteDistribution(data);
  const totalVoted = distribution.reduce((total, segment) => total + segment.voted, 0);

  return <Card><CardHeader><CardTitle>Distribución de votantes por sexo</CardTitle><CardDescription>Porcentaje de quienes votaron, distribuido según el sexo registrado en el censo.</CardDescription></CardHeader><CardContent>{totalVoted > 0 ? <><div className="h-[280px]" role="img" aria-label={`Distribución de ${totalVoted} votantes por sexo`}><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={distribution} dataKey="voted" nameKey="label" cx="50%" cy="50%" innerRadius={58} outerRadius={100} paddingAngle={2} label={({ percent }) => `${Math.round((percent ?? 0) * 100)}%`} labelLine>{distribution.map((segment) => <Cell key={segment.label} fill={segment.color} />)}</Pie><Tooltip formatter={(value: number) => [`${value} voto(s)`, 'Votaron']} /></PieChart></ResponsiveContainer></div><div className="grid gap-2 sm:grid-cols-2">{distribution.map((segment) => <div key={segment.label} className="flex items-center justify-between gap-3 rounded-md border px-3 py-2 text-sm"><span className="flex min-w-0 items-center gap-2"><span className="h-3 w-3 shrink-0 rounded-full" style={{ backgroundColor: segment.color }} /><span className="truncate">{segment.label}</span></span><span className="shrink-0 font-semibold tabular-nums">{segment.percentage}% <span className="font-normal text-muted-foreground">({segment.voted})</span></span></div>)}</div></> : <div className="flex h-[320px] items-center justify-center text-sm text-muted-foreground">Aún no hay votos para representar.</div>}</CardContent></Card>;
}

export function SegmentTable({ title, segmentLabel, data, onExport }: { title: string; segmentLabel: string; data: CopasstSegment[]; onExport?: () => void }) {
  return <Card><CardHeader className="flex flex-col items-start justify-between gap-4 sm:flex-row"><div><CardTitle>{title}</CardTitle><CardDescription>Comparativo de habilitados, votantes y porcentaje de participación.</CardDescription></div>{onExport && <Button variant="outline" size="sm" onClick={onExport} disabled={data.length === 0}><Download className="mr-2 h-4 w-4" />Exportar Excel</Button>}</CardHeader><CardContent className="px-0 sm:px-6"><Table><TableHeader><TableRow><TableHead className="min-w-48">{segmentLabel}</TableHead><TableHead className="min-w-52">Participación</TableHead><TableHead className="text-right">Habilitados</TableHead><TableHead className="text-right">Votaron</TableHead><TableHead className="text-right">Pendientes</TableHead></TableRow></TableHeader><TableBody>{data.map((segment) => { const participation = segment.eligible > 0 ? Math.round(segment.voted * 1000 / segment.eligible) / 10 : 0; return <TableRow key={segment.label}><TableCell className="max-w-72 font-medium"><span className="line-clamp-2">{segment.label}</span></TableCell><TableCell><div className="flex min-w-44 items-center gap-3"><div className="h-2.5 flex-1 overflow-hidden rounded-full bg-slate-200" role="img" aria-label={`${participation}% de participación`}><div className="h-full rounded-full bg-primary transition-[width]" style={{ width: `${Math.min(100, participation)}%` }} /></div><span className="w-12 text-right text-xs font-semibold tabular-nums">{participation}%</span></div></TableCell><TableCell className="text-right font-medium tabular-nums">{segment.eligible}</TableCell><TableCell className="text-right font-semibold text-primary tabular-nums">{segment.voted}</TableCell><TableCell className="text-right tabular-nums">{Math.max(0, segment.eligible - segment.voted)}</TableCell></TableRow>; })}{data.length === 0 ? <TableRow><TableCell colSpan={5} className="h-24 text-center text-muted-foreground">No hay información disponible para esta dimensión.</TableCell></TableRow> : null}</TableBody></Table></CardContent></Card>;
}
