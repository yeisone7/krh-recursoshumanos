...
const mocks = vi.hoisted(() => ({ canApprove: true, from: vi.fn() }));
vi.mock('@/integrations/supabase/client', () => ({ supabase: { from: mocks.from } }));
vi.mock('@/contexts/AuthContext', () => ({ useAuth: () => ({
  currentCompanyId: 'company', user: { id: 'requester' },
  hasPermission: (module: string, action: string) => module === 'correction_tickets' && (action === 'create' || (action === 'approve' && mocks.canApprove)),
}) }));
...
vi.mock('@/components/payroll/CorrectionDashboard', () => ({ CorrectionDashboard: () => null }));
