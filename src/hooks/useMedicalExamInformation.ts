...
export function useMedicalExamInformation() {
  ...
  const cacheKey = `medical-exam-information:${currentCompanyId}`;
  const query = useQuery({ queryKey, enabled: Boolean(currentCompanyId), staleTime: 15 * 60 * 1000, gcTime: 60 * 60 * 1000, refetchOnMount: false, initialData: () => getSelectionCatalogCache<MedicalExamInformation[]>(cacheKey)?.data, initialDataUpdatedAt: () => getSelectionCatalogCache<MedicalExamInformation[]>(cacheKey)?.updatedAt, queryFn: async (): Promise<MedicalExamInformation[]> => { const { data, error } = await supabase.from(table).select('*, operation_centers(id, name)').eq('company_id', currentCompanyId).order('created_at', { ascending: false }); if (error) throw error; const records = (data || []) as unknown as MedicalExamInformation[]; setSelectionCatalogCache(cacheKey, records); return records; } });
