...
batch_dir = r'c:\Users\YEISON\Proyectos AI\krh-recursoshumanos\combined_batches'
results = {}

for filename in os.listdir(batch_dir):
    if filename.startswith('part_') and filename.endswith('.sql'):
            ...
            for doc in missing_docs:
                if doc in content:
                    if filename not in results:
                        results[filename] = []
                    results[filename].append(doc)

for batch, docs in sorted(results.items()):
    print(f"{batch}: {len(docs)} missing docs")
