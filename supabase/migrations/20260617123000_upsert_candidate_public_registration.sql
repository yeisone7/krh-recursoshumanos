-- Upsert candidate public registrations by document within the linked vacancy.
-- The lookup RPC also preloads existing candidate data for the same company so
-- candidates do not have to retype information already stored in the system.
...
DECLARE
  ...
  v_vacancy_company_id uuid;
  v_document_type document_type;
  v_family_members jsonb := '[]'::jsonb;
  v_same_vacancy boolean := false;
BEGIN
  ...
  SELECT company_id INTO v_vacancy_company_id
  FROM vacancies
  WHERE id = v_token_row.vacancy_id;

  IF NOT FOUND THEN
    RETURN json_build_object('success', false, 'error', 'La vacante del enlace no existe');
  END IF;

  v_document_type := COALESCE(NULLIF(p_document_type, ''), 'CC')::document_type;

  SELECT * INTO v_candidate
  FROM candidates
  WHERE vacancy_id = v_token_row.vacancy_id
    AND company_id = v_vacancy_company_id
    AND document_type = v_document_type
    AND lower(btrim(document_number)) = lower(btrim(p_document_number))
  ORDER BY updated_at DESC NULLS LAST, created_at DESC
  LIMIT 1;

  v_same_vacancy := FOUND;

  IF NOT FOUND THEN
    SELECT * INTO v_candidate
    FROM candidates
    WHERE company_id = v_vacancy_company_id
      AND document_type = v_document_type
      AND lower(btrim(document_number)) = lower(btrim(p_document_number))
    ORDER BY updated_at DESC NULLS LAST, created_at DESC
    LIMIT 1;
  ...
  RETURN json_build_object(
    'success', true,
    'found', true,
    'same_vacancy', v_same_vacancy,
    'candidate', row_to_json(v_candidate),
...
BEGIN
  ...
  IF v_token_row.company_id IS DISTINCT FROM v_vacancy_company_id THEN
    UPDATE self_registration_tokens
    SET company_id = v_vacancy_company_id,
        updated_at = now()
  ...
  IF NULLIF(btrim(COALESCE(p_document_number, '')), '') IS NOT NULL THEN
  ...
  IF v_existing_candidate_id IS NOT NULL THEN
    ...
    SET
      ...
      document_issue_date = CASE WHEN p_document_issue_date IS NOT NULL AND p_document_issue_date != '' THEN p_document_issue_date::date ELSE NULL END,
      ...
      is_first_job = COALESCE(p_is_first_job, false),
      is_head_of_household = COALESCE(p_is_head_of_household, false),
      disability_type = p_disability_type,
      ethnic_group = p_ethnic_group,
      is_conflict_victim = COALESCE(p_is_conflict_victim, false),
      is_demobilized = COALESCE(p_is_demobilized, false),
      identification_type_id = p_identification_type_id,
      updated_at = now()
    WHERE id = v_existing_candidate_id
  ...
  ELSE
    INSERT INTO candidates (
      ...
      v_vacancy_company_id, v_token_row.vacancy_id, p_first_name, p_last_name, v_document_type, p_document_number,
  ...
  IF (v_token_row.enabled_fields ? 'familyMembers')
    AND jsonb_typeof(COALESCE(p_family_members, '[]'::jsonb)) = 'array'
  THEN
    DELETE FROM candidate_family_members
  ...
  RETURN json_build_object(
    ...
    'vacancy_id', v_token_row.vacancy_id,
    'company_id', v_vacancy_company_id,
    'existing', v_existing_candidate_id IS NOT NULL
  );
END;
$$;
