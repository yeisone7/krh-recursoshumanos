-- Fetches employee data if the token is valid for the employee's company
...
BEGIN
    ...
    IF NOT FOUND OR v_token_row.is_used OR v_token_row.expires_at < now() OR v_token_row.target_type != 'employee' THEN
...
-- Updates an existing employee using a valid registration token
...
BEGIN
  ...
  -- Update contact info (mark old as not current and insert new if different)
  -- For simplicity in this public form, we just update the current active record
