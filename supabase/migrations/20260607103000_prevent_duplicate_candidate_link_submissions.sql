-- Prevent candidate self-registration links from creating duplicate candidates.
-- Reusable links can be used by multiple people, but the same document in the
-- same vacancy updates the existing candidate instead of inserting another row.
-- One-use links are locked while submitting, so rapid double submissions still
-- respect the first successful use.
