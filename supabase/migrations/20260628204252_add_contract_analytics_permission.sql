WITH parent_module AS (
  SELECT id FROM public.modules WHERE code = 'contratos'
)
INSERT INTO public.modules (code, name, icon, sort_order, parent_id)
SELECT 'analitica_contratos', 'Analitica de Contratos', 'BarChart3', 41, parent_module.id
FROM parent_module
...
CROSS JOIN (
  VALUES
    ('view', 'Analitica de Contratos - Ver'),
    ('export', 'Analitica de Contratos - Exportar')
) AS permission_seed(action_value, description_value)
WHERE m.code = 'analitica_contratos'
...
WHERE cr.is_system = true
  AND m.code = 'analitica_contratos'
