-- Make documents uploaded during Selection and Vacancies visible once the
-- candidate is hired as an employee.

ALTER TABLE public.candidate_documents
  ADD COLUMN IF NOT EXISTS document_type public.employee_document_type NOT NULL DEFAULT 'proceso_seleccion',
  ADD COLUMN IF NOT EXISTS expiry_date date;

CREATE INDEX IF NOT EXISTS idx_candidate_documents_candidate
ON public.candidate_documents(candidate_id);

CREATE INDEX IF NOT EXISTS idx_candidate_documents_file_url
ON public.candidate_documents(file_url);
...
BEGIN
  ...
  SET
    ...
    document_type = COALESCE(NEW.document_type, 'proceso_seleccion'::public.employee_document_type),
  ...
  IF NOT FOUND THEN
    ...
    VALUES (
      ...
      COALESCE(NEW.document_type, 'proceso_seleccion'::public.employee_document_type),
...
SELECT
  ...
  COALESCE(cd.document_type, 'proceso_seleccion'::public.employee_document_type),
